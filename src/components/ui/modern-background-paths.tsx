...
function GeometricPaths() {
  ...
  for (let x = 0; x < 20; x++) {
    for (let y = 0; y < 12; y++) {
      const seed = x * 100 + y; // Deterministic seed
      if (seededRandom(seed) > 0.7) {
        paths.push({
          ...
          delay: seededRandom(seed + 1000) * 5,
        })
      }
    }
  }
...
function NeuralPaths() {
  const nodes = Array.from({ length: 50 }, (_, i) => ({
    x: seededRandom(i * 123) * 800,
    y: seededRandom(i * 456) * 600,
    id: `node-${i}`
  }))
  ...
  nodes.forEach((node, i) => {
    const nearbyNodes = nodes.filter((other, j) => {
      ...
      return distance < 120 && seededRandom(i * 789 + j) > 0.6
    })
    
    nearbyNodes.forEach((target, index) => {
      connections.push({
        id: `conn-${i}-${target.id}`,
        d: `M${node.x},${node.y} L${target.x},${target.y}`,
        delay: seededRandom(i * 999 + index * 111) * 10
      })
    })
  })
